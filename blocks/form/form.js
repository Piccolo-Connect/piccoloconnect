...
function updateLabelForSuccess(formContainer) {
  const loaderLabel = formContainer.querySelector('.form-loader-label');
  if (loaderLabel) {
    loaderLabel.textContent = 'Successfully submitted!';
  }
}
...
export async function submitForm(form) {
  ...
  // Once response is received, check the status
  if (!resp.ok) {
    // Update the loader to show error state
    updateLoaderForError(formContainer);
    delayTime = 4000; // 4 seconds for errors
  } else {
    form.reset();
    // Update the loader to show success state
    updateLabelForSuccess(formContainer);
    delayTime = 2000; // 2 seconds for success
  }

  // Remove the loader after the specified delay
  setTimeout(() => {
...
export function updateSubmitButtonState(form) {
